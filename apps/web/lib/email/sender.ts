import { logError, logWarn } from "../server-log";

export type TransactionalEmailInput = {
  to: string;
  subject: string;
  html: string;
  text: string;
};

export type EmailSendResult =
  | { ok: true; provider: "resend"; id: string | null }
  | {
      ok: false;
      provider: "resend";
      code: "EMAIL_PROVIDER_NOT_CONFIGURED" | "EMAIL_PROVIDER_REJECTED" | "EMAIL_PROVIDER_ERROR";
      error: string;
      skipped?: boolean;
    };

export interface EmailSender {
  provider: "resend";
  send(input: TransactionalEmailInput): Promise<EmailSendResult>;
}

type ResendSendResponse = {
  id?: string;
  error?: { message?: string };
};

const DEFAULT_FROM_NAME = "Memorioso";

export function formatResendFromAddress(rawFrom: string, rawFromName?: string) {
  const from = rawFrom.trim();
  if (!from) return "";
  if (from.includes("<") && from.includes(">")) return from;

  const fromName = (rawFromName || DEFAULT_FROM_NAME).trim() || DEFAULT_FROM_NAME;
  return `${fromName} <${from}>`;
}

function getResendConfig() {
  return {
    apiKey: process.env.RESEND_API_KEY || "",
    from: process.env.RESEND_FROM_EMAIL || "",
    fromName: process.env.RESEND_FROM_NAME || DEFAULT_FROM_NAME
  };
}

export function createResendSender(): EmailSender {
  return {
    provider: "resend",
    async send(input) {
      const config = getResendConfig();
      if (!config.apiKey || !config.from) {
        logWarn("auth_email_config_missing", {
          hasApiKey: Boolean(config.apiKey),
          hasFromEmail: Boolean(config.from)
        });

        return {
          ok: false,
          provider: "resend",
          code: "EMAIL_PROVIDER_NOT_CONFIGURED",
          skipped: true,
          error: "Email provider not configured."
        };
      }

      try {
        const fromAddress = formatResendFromAddress(config.from, config.fromName);

        const response = await fetch("https://api.resend.com/emails", {
          method: "POST",
          headers: {
            "content-type": "application/json",
            authorization: `Bearer ${config.apiKey}`
          },
          body: JSON.stringify({
            from: fromAddress,
            to: [input.to],
            subject: input.subject,
            html: input.html,
            text: input.text
          })
        });

        const payload = (await response.json().catch(() => ({}))) as ResendSendResponse;

        if (!response.ok) {
          logError("auth_email_send_failed", {
            status: response.status,
            statusText: response.statusText,
            message: payload.error?.message || "unknown"
          });

          return {
            ok: false,
            provider: "resend",
            code: "EMAIL_PROVIDER_REJECTED",
            error: payload.error?.message || "Failed to send email."
          };
        }

        return {
          ok: true,
          provider: "resend",
          id: payload.id || null
        };
      } catch (error) {
        logError("auth_email_send_exception", error);
        return {
          ok: false,
          provider: "resend",
          code: "EMAIL_PROVIDER_ERROR",
          error: error instanceof Error ? error.message : "Failed to send email."
        };
      }
    }
  };
}

export async function sendEmail(input: TransactionalEmailInput) {
  const sender = createResendSender();
  return sender.send(input);
}
