...
type ExportRunMap = Record<
  ExportTarget,
  { status: TargetRunState; error?: string }
>;
...
function summarizePreflightBlock(preflight: ExportPreflightResponse) {
  ...
  if (blockingCount > 0) {
    parts.push(
      `${blockingCount} preflight issue${blockingCount === 1 ? "" : "s"}`,
    );
  }
  if (contractErrorCount > 0) {
    parts.push(
      `${contractErrorCount} contract error${contractErrorCount === 1 ? "" : "s"}`,
    );
  }
  return parts.join(", ");
}

function resolveBillingActionHint(
...
export function ExportModal({
  ...
  const [preflightByTarget, setPreflightByTarget] = useState<
    Partial<Record<ExportTarget, ExportPreflightResponse>>
  >({});
  ...
  async function runTargetExport(target: ExportTarget, preview: boolean) {
    ...
    if (!preflight.canProceed) {
      setRunState((current) => ({
        ...
        [target]: {
          status: "blocked",
          error: summarizePreflightBlock(preflight),
        },
      }));
      const blockingCount = preflight.blockingIssues.length;
      const contractErrorCount = preflight.contractErrors?.length ?? 0;
    ...
    if (!result.ok) {
      setRunState((current) => ({
        ...current,
        [target]: { status: "failed", error: result.error.message },
      }));
    ...
    setResults((current) => [
      {
        ...
        warningsCount:
          ...
          (result.meta.warningsFromPreflight?.length ?? 0),
      },
      ...current.filter(
        (item) =>
          !(
            item.target === target && item.exportHash === result.meta.exportHash
          ),
      ),
    ]);
  ...
  async function saveExportSettings() {
    ...
    onExportSettingsChange?.(
      normalizeStorybookExportSettingsV1(result.data.settings),
    );
    setGlobalError(null);
  }
              ...
              <div className="space-y-4">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setSettingsDraft((current) => ({
                            ...current,
                            exportTargets: {
                              ...current.exportTargets,
                              digitalPdf: event.target.checked,
                            },
                          }))
                        }
                      />
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setSettingsDraft((current) => ({
                            ...
                            exportTargets: {
                              ...current.exportTargets,
                              hardcopyPdf: event.target.checked,
                            },
                          }))
                        }
                      />
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setSettingsDraft((current) => ({
                            ...
                            printPreset: {
                              ...current.printPreset,
                              safeAreaPadding: Number(event.target.value),
                            },
                          }))
                        }
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setSettingsDraft((current) => ({
                            ...
                            printPreset: {
                              ...current.printPreset,
                              minImageWidthPx: Number(event.target.value),
                            },
                          }))
                        }
                        className="mt-1 h-9 w-full rounded-lg border border-white/15 bg-black/25 px-2 text-sm text-white"
                ...
                <div className="rounded-xl border border-white/10 bg-white/[0.02] p-3">
                  ...
                  <div className="space-y-2 text-xs">
                    {(["DIGITAL_PDF", "HARDCOPY_PRINT_PDF"] as const).map(
                      (target) => (
                        <div
                          key={target}
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button
                    type="button"
                    loading={isBusy}
                    onClick={() => void generateSelected(false)}
                  >
                    {selection === "BOTH"
